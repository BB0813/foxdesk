...
- **双引擎** — 档案级选择 Camoufox（Firefox）或 Chromium（Playwright/Patchright，auto 优先 Patchright）
...
- **代理环境匹配** — 出口 IP 归属检测（住宅/机房）、一键匹配时区/语言、批量套用环境设置
...
- 安装包：`installer_output/FoxDesk-1.4.5-Setup.exe`
...
- 从旧版升级：建议直接装 1.4.5；Program Files 旧安装请先卸载
...
| 前端 | 原生 HTML/CSS/JS |
| 浏览器 | Camoufox（Firefox）+ Chromium（Playwright / Patchright） |
